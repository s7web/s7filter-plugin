									...
									<?php foreach ( $pages as $id => $page ) : ?>
										...
										<form action="<?php esc_url( admin_url() ); ?>admin-post.php" method="post">
											...
											<input type="radio" <?php if ( $page['settings'] && 'both' === $page['settings']['filter'] ) {
												echo 'checked';
											} ?> value="both" name="otrs-data[filter]"> Both categories and tags for
											posts
											<br>
											<input type="radio" <?php if ( $page['settings'] && 'none' === $page['settings']['filter'] ) {
												echo 'checked';
											} ?> value="none" name="otrs-data[filter]"> None
											<br>
											<h4>Maximum posts per this page:</h4>
											<div class="wppf-fold">
												<input type="text" value="<?php if ( $page['settings'] ) {
													echo $page['settings']['per_page'];
												} ?>" class="4_posts_per_page" size="3" name="otrs-data[per_page]">
											</div>
											...
											<div class="wppf-fold">
												<select value="default" size="1" class="4_template wppf-controls"
													...
													<option <?php if ( $page['settings'] && 'default-thumb-enabled' == $page['settings']['template'] ) {
														echo 'selected';
													} ?> value="default-thumb-enabled">default-thumb-enabled
													</option>
											...
											<div class="wppf-fold">
												<input type="text" value="<?php if ( $page['settings'] ) {
													echo $page['settings']['dateformat'];
												} ?>" class="4_dateformat wppf-controls" id="wppf_opts[4][dateformat]"
													   name="otrs-data[dateformat]">
											</div>
											<h4><?php esc_html_e( 'Heading tag for the posts on this page:', 's7design-filter' ); ?></h4>
											<div class="wppf-fold">
												<input type="text" value="<?php if ( $page['settings'] ) {
													echo $page['settings']['heading'];
												} ?>" class="4_heading_tag wppf-controls" name="otrs-data[heading]">
											</div>
											<h4><?php esc_html_e( 'Heading class for the posts on this page:', 's7design-filter' ); ?></h4>
											<div class="wppf-fold">
												<input type="text" value="<?php if ( $page['settings'] ) {
													echo $page['settings']['heading_class'];
												} ?>" class="4_heading_class wppf-controls"
